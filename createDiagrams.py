...
def topGenes(sumDict, fileName,number=20,value=5, logFile=None,textField=0):
        ...
        counts=OrderedDict(sorted(list(sumDict.items()), key=lambda t: t[1][value], reverse=True)[:number])
        barNameTuple=()
        valueMatrix=[[]]
...
def createDiagramms(output, geneNumber=20,logFile=None,textField=0):
        ...
        valueMatrix=[list(counts1.values()), list(counts2.values())]
